...
function ChildCard({
  child, onChange, onRemove, canRemove,
}: {
  child: ChildDraft
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
        <div>
          <FieldLabel>First name</FieldLabel>
          <input style={inputStyle} value={child.firstName} onChange={(e) => onChange({ firstName: e.target.value })} />
        </div>
        <div>
          <FieldLabel>Last name</FieldLabel>
          <input style={inputStyle} value={child.lastName} onChange={(e) => onChange({ lastName: e.target.value })} />
        </div>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
        <div>
          ...
          <input
            style={inputStyle} type="date" value={child.dob}
...
function AthletesStep({
  children, onChangeChild, onAddChild, onRemoveChild,
}: {
  children: ChildDraft[]
  onChangeChild: (localId: string, patch: Partial<ChildDraft>) => void
  onAddChild: () => void
  ...
  return (
    <div>
      <div style={{ fontSize: '13px', color: T.ink3, fontFamily: "'Hanken Grotesk', sans-serif", marginBottom: '16px' }}>
        Add each athlete you'll be booking sessions for.
      </div>
      {children.map((child) => (
        <ChildCard
          key={child.localId}
          child={child}
          onChange={(patch) => onChangeChild(child.localId, patch)}
      ...
      <button
        ...
        style={{
          ...
          background: 'transparent', color: T.ink3, fontSize: '14px', fontWeight: 600,
...
function ConsentStep({
  children, agreed, setAgreed, error,
}: {
  children: ChildDraft[]
  agreed: boolean
  ...
  return (
    <div>
      <div style={{
        background: 'rgba(255,255,255,0.90)', backdropFilter: 'blur(8px)', WebkitBackdropFilter: 'blur(8px)',
        border: '1px solid rgba(0,0,0,0.08)', borderRadius: '14px', padding: '18px', marginBottom: '20px',
      }}>
        {children.map((child, i) => {
          const age = computeAge(child.dob)
          return (
            <div key={child.localId} style={{
              display: 'flex', justifyContent: 'space-between', alignItems: 'center',
              padding: '10px 0', borderBottom: i < children.length - 1 ? `1px solid ${T.line}` : 'none',
            }}>
              <div style={{ fontFamily: "'Hanken Grotesk', sans-serif", fontSize: '14px', fontWeight: 600, color: T.ink }}>
                {child.firstName} {child.lastName}
              </div>
              <div style={{ fontFamily: "'Hanken Grotesk', sans-serif", fontSize: '13px', color: T.ink3 }}>
                {age != null ? `${age} yrs · ` : ''}{child.sport}
              </div>
            </div>
          )
        })}
      ...
      <label style={{ display: 'flex', alignItems: 'flex-start', gap: '10px', cursor: 'pointer' }}>
        ...
        <span style={{ fontSize: '13px', color: T.ink2, fontFamily: "'Hanken Grotesk', sans-serif", lineHeight: 1.5 }}>
          I confirm I am the parent or legal guardian of the athlete(s) listed above and I agree to
          FARM's Terms of Service and Privacy Policy on their behalf.
        </span>
      </label>
...
export default function ParentOnboardingPage() {
  ...
  const accountValid = firstName.trim().length > 0
    && lastName.trim().length > 0
    && email.trim().includes('@')
    && password.length >= 8

  const athletesValid = children.length > 0 && children.every((c) =>
    c.firstName.trim().length > 0 && c.lastName.trim().length > 0 && c.dob !== '' && c.sport !== ''
  )
  ...
  async function handleCreateAccount() {
    ...
    const { data, error } = await supabase.auth.signUp({
      email: email.trim(),
      password,
      options: { data: { role: 'parent', name: fullName } },
    })
    setSubmitting(false)
    if (error || !data.user) {
      setAccountError(error?.message ?? 'Something went wrong. Please try again.')
      return
    }
  ...
  function addChild() {
    setChildren((prev) => [...prev, newChildDraft()])
  }

  function removeChild(localId: string) {
    setChildren((prev) => (prev.length > 1 ? prev.filter((c) => c.localId !== localId) : prev))
  }

  async function handleFinish() {
    if (!userId) return
    if (!agreed) { setConsentError('Please confirm you agree to continue.'); return }
    setSubmitting(true)
    ...
    try {
      ...
      setConsentError(e instanceof Error ? e.message : 'Something went wrong. Please try again.')
    }
    setSubmitting(false)
  }

  function handleContinue() {
    if (step === 0 && accountValid) void handleCreateAccount()
  ...
  return (
    <>
      {step === 0 && (
        <WizardShell
          step={0} title="Create your account" subtitle="Let's get you set up on FARM."
          onBack={null} onContinue={handleContinue} canContinue={accountValid}
      ...
      {step === 1 && (
        <WizardShell
          step={1} title="Add your athletes" subtitle="Who are you booking sessions for?"
          onBack={null} onContinue={handleContinue} canContinue={athletesValid}
          submitting={false} isLastStep={false}
        >
          <AthletesStep
            children={children} onChangeChild={updateChild} onAddChild={addChild} onRemoveChild={removeChild}
          />
        </WizardShell>
      )}
      {step === 2 && (
        <WizardShell
          step={2} title="Consent & review" subtitle="One last thing before you're done."
          onBack={() => setStep(1)} onContinue={handleContinue} canContinue={agreed}
          submitting={submitting} isLastStep
        >
          <ConsentStep children={children} agreed={agreed} setAgreed={setAgreed} error={consentError} />
        </WizardShell>
      )}
    </>
  )
